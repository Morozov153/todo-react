...
const deleteAllTasks = useCallback(() => {
    const isConfirmed = confirm("Ты действительно хочешь удалить все задачи?")
    if (isConfirmed) 
...
const clearSearchQuery = searchQuery.trim().toLowerCase()
    return clearSearchQuery.length > 0 ? tasks.filter(({title}) => title.toLowerCase().includes(clearSearchQuery)) : null},[searchQuery,tasks])
